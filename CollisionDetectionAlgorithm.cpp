...
int32_t DS9AndICS2distinguishableSet[][5]={	{0b001000,-1,-1,0b001011,0b001011}, /* DS9_1 ICS2_1*/
									{0b000000,-1,-1,0b011111,0b011111}}; /* DS9_2 ICS2_2*/
int32_t ICS3distinguishableSet[][5]={{0b000001,0b000010,0b011101,0b011110,0b011111}, /* ICS3_1* */
									{0b000000,-1,-1,0b011000,0b011000}, /* ICS3_1** */
									{0b000000,-1,-1,0b011111,0b011111}, /* ICS3_2* */
									{0b010000,-1,-1,0b010110,0b110110}, /* ICS3_2** */
									{-1,0b000100,-1,-1,0b001100},       /* ICS3_3* */
									{0b100000,-1,-1,0b111000,0b111000}};/* ICS3_3** */
...
int32 newIndividualCheckedSet[][5]={{-1,0x9,0xc,-1,0x1d}, /* NICS1*** */
									{0b0,-1,-1,0b011110,0b011110}, /* NICS1* */
									{0b100000,-1,-1,0b101100,0b101100}, /* NICS1** */
									{0x4,-1,-1,-1,0x4}, /* NICS2*** */
									{0b0,-1,-1,0b011111,0b011111}, /* NICS2* */
									{0x20000,-1,-1,-1,0x20000}, /* NICS3*** */
									{0b00100,-1,-1,0b001011,0b001011}, /* NICS3* */
									{0b0,-1,-1,0b011000,0b011000}}; /* NICS3** */
...
void Compression_Modify(uint32 ihv[], uint32 cv[], uint32 scv[],uint32 ascv[], const uint32 block[]){
	uint32 a = ihv[0];
	uint32 b = ihv[1];
	uint32 c = ihv[2];
	uint32 d = ihv[3];
	...
	STEPFUNCTION(G, a, b, c, d,  5, 0x21e1cde6, block[ 9]);	/* 24 */
	STEPFUNCTION(G, d, a, b, c,  9, 0xc33707d6, block[14]);	/* 25 */
	//output Q26[31], Q26[27],Q26[22], Q26[17], Q26[11] and Q26[8]
	GetAdditionalBits(ascv[0], ascv[1], ascv[2], ascv[3], ascv[4],ascv[5], d);

	STEPFUNCTION(G, c, d, a, b, 14, 0xf4d50d87, block[ 3]);	/* 26 */
	//output Q27[31], Q27[27],Q26[22], Q27[17], Q27[11] and Q27[8]
	GetAdditionalBits(ascv[0], ascv[1], ascv[2], ascv[3], ascv[4],ascv[5], c);

	STEPFUNCTION(G, b, c, d, a, 20, 0x455a14ed, block[ 8]);	/* 27 */
	//output Q28[31], Q28[27],Q26[22], Q28[17], Q28[11] and Q28[8]
	GetAdditionalBits(ascv[0], ascv[1], ascv[2], ascv[3], ascv[4],ascv[5], b);

	STEPFUNCTION(G, a, b, c, d,  5, 0xa9e3e905, block[13]);	/* 28 */
	//output Q29[31], Q29[27],Q26[22], Q29[17], Q29[11] and Q29[8]
	GetAdditionalBits(ascv[0], ascv[1], ascv[2], ascv[3], ascv[4],ascv[5], a);

	STEPFUNCTION(G, d, a, b, c,  9, 0xfcefa3f8, block[ 2]);	/* 29 */
	//output Q30[31], Q30[27],Q26[22], Q30[17], Q30[11] and Q30[8]
	GetAdditionalBits(ascv[0], ascv[1], ascv[2], ascv[3], ascv[4],ascv[5], d);

	STEPFUNCTION(G, c, d, a, b, 14, 0x676f02d9, block[ 7]);	/* 30 */
	//output Q31[31], Q31[27],Q26[22], Q31[17], Q31[11] and Q31[8]
	GetAdditionalBits(ascv[0], ascv[1], ascv[2], ascv[3], ascv[4],ascv[5], c);
...
bool DSFunction_1(const uint32 cv[], const uint32 ihv[], const uint32 block[]){
	...
	ReverseCompression(tihv, tcv, tblock); //计算加上消息差分后 相应IHV (k+1)的值，比较
	if(Equal(tihv, ihv)){ 
		return true;
	}
...
bool DSFunction_9(const uint32 cv[], const uint32 ihv[], const uint32 ascv[],const uint32 block[]){
	uint32 tcv[4], tihv[4], tblock[16];
	tcv[0] = cv[0];		//copy cv to cv37, cv38, cv39, cv40
	tcv[1] = cv[1];
	tcv[2] = cv[2];
	...
	memcpy(tblock, block, 64);	//copy message block
	uint8_t col=0,auxcol=0;
	col=ascv[1] & (0b11); //first row key bits are cv29[27], cv30[27]
	auxcol=ascv[0] & (0b11); //second row key bits are cv29[31], cv30[31]
	if (DS9AndICS2distinguishableSet[0][col]== (ascv[1] & DS9AndICS2distinguishableSet[0][4] ))
	{
	...
	else if (DS9AndICS2distinguishableSet[1][auxcol]== (ascv[0] & DS9AndICS2distinguishableSet[1][4] ))
	{
		tblock[2]=block[2]; //eliminate difference of m2 and m14
		tblock[14]=block[14];
...
bool DistinguishableSetCheckAlgorithm(const uint32 ihv[], const uint32 cv[], const uint32 scv[],const uint32 ascv[], const uint32 block[]){
	bool isCollisionBlock = false;
	int col = scv[0] & 0x00000003;	//key bits, the least 2 bits of scv[0] //brief 关键比特换成高位也是一样的，同样可以决定条件选哪种
	for(int i = 0; i < 14; i++){
		if(distinguishableSet[i][col] == (scv[0] & distinguishableSet[i][4])){	//find a candidate DS Element 
			switch(i){		//Entry of DS Element
				...
				case 8: isCollisionBlock = DSFunction_9(cv, ihv, ascv, block); break;
...
bool IndividualSetCheckAlgorithm(const uint32 ihv[], const uint32 cv[], const uint32 scv[],const uint32 ascv[], const uint32 block[]){
	uint32 tcv[4], tihv[4], tblock[16];
	uint16 col = 0, auxcol = 0, thirdcol=0;
	tcv[0] = cv[0];		//copy cv to cv37, cv38, cv39, cv40
	tcv[1] = cv[1];
	tcv[2] = cv[2];
	tcv[3] = cv[3];
	...
	if(individualCheckedSet[1][col] == (scv[3] & individualCheckedSet[1][4])){
		...
		{
			tblock[14]=block[14]; //eliminate difference of m14
			//block difference m14 = 2^31, cover 1 case
			tblock[14] += 0x80000000;
			//block difference m11 = +2^15, m4 = m14 = 2^31, cover 1 case
			tblock[11] += 0x00008000;
	...
	if(individualCheckedSet[2][col] == (scv[4] & individualCheckedSet[2][4])){
		memcpy(tblock, block, 64);	//copy block to temporary variables
		col=ascv[0] & (0b110); //first row key bits are cv28[31], cv29[31]
		auxcol=ascv[2] & (0b11000); //second row key bits are cv26[22], cv27[22]
		if ((ICS3distinguishableSet[0][col]== (ascv[0] & ICS3distinguishableSet[0][4]))
		&& (ICS3distinguishableSet[1][auxcol]==(ascv[2] & ICS3distinguishableSet[1][4])))
		{
			//block difference m5 = 2^31, cover 1 case
			tblock[5] += 0x80000000;
			ReverseCompression(tihv, tcv, tblock);
		...
		col=ascv[0] & (0b11); //first row key bits are cv29[31], cv30[31]
		auxcol=ascv[4] & (0b110); //second row key bits are cv26[11], cv27[11]
		if ((ICS3distinguishableSet[2][col]== (ascv[0] & ICS3distinguishableSet[2][4]))
		&& (ICS3distinguishableSet[3][auxcol]==(ascv[4] & ICS3distinguishableSet[3][4])))
		{
			tblock[5]=block[5];  //eliminate difference of m5
			//block difference m5 = 2^31, m11 = 2^31, cover 1 case
			tblock[5] += 0x80000000;
			tblock[11] += 0x80000000;
			ReverseCompression(tihv, tcv, tblock);
		...
		auxcol=ascv[3] & (0b11000); //second row key bits are cv26[17], cv27[17]
		if ((ICS3distinguishableSet[4][1]== (ascv[0] & ICS3distinguishableSet[4][4])) //just one value 
		&& (ICS3distinguishableSet[5][auxcol]==(ascv[3] & ICS3distinguishableSet[5][4])))
		{
			tblock[5]=block[5];  //eliminate difference of m5
			tblock[11]=block[11];  
	...
	//NICS1 Element Check, cover 2 cases
	col = scv[1] & (0x00000005);	//key bits are cv56[31], cv58[31]
	auxcol = ascv[4] & (0b1100);    //key bits are cv28[11], cv29[11]
	if((newIndividualCheckedSet[0][col] == (scv[0] & newIndividualCheckedSet[0][4]))
	&& (newIndividualCheckedSet[2][auxcol] == (ascv[4] & newIndividualCheckedSet[2][4])))
	{
		memcpy(tblock, block, 64);	//copy message block
		//block difference m8 = +2^25, cover 1 case
		tblock[8] += 0x02000000;
	...
	//NICS2 Element Check, cover 1 cases
	col = scv[1] & (0x00000005);	//key bits are cv56[5], cv58[5]
	auxcol = ascv[0] & (0b1100);    //key bits are cv28[31], cv29[31]
	thirdcol = ascv[3] & (0b1100);  //key bits are cv28[17], cv29[17]
	if((newIndividualCheckedSet[0][col] == (scv[1] & newIndividualCheckedSet[0][4]))
	&& (newIndividualCheckedSet[1][auxcol] == (ascv[0] & newIndividualCheckedSet[1][4]))
	&& (newIndividualCheckedSet[2][thirdcol] == (ascv[3] & newIndividualCheckedSet[2][4])))
	{
		memcpy(tblock, block, 64);	//copy message block
		//block difference m8 = 2^31, cover 1 case
		tblock[8] += 0x80000000;
	...
	//NICS3 Element Check, cover 2 cases
	auxcol = ascv[0] & (0b1100);    //key bits are cv28[31], cv29[31]
	if((newIndividualCheckedSet[3][0] == (scv[2] & newIndividualCheckedSet[3][4])) // has only one value
	&& (newIndividualCheckedSet[4][auxcol] == (ascv[0] & newIndividualCheckedSet[4][4])))
	{
		memcpy(tblock, block, 64);	//copy message block
		//block difference m6 = +2^8,m9=m15=2^31 cover 1 case
		tblock[6] += 0x00000100;
	...
	//NICS4 Element Check, cover 2 cases
	auxcol = ascv[1] & (0b11);    //key bits are cv29[27], cv30[27]
	thirdcol = ascv[5] & (0b11000);  //key bits are cv27[8], cv28[8]
	if((newIndividualCheckedSet[5][0] == (scv[1] & newIndividualCheckedSet[5][4])) // has only one value
	&& (newIndividualCheckedSet[6][auxcol] == (ascv[1] & newIndividualCheckedSet[6][4]))
	&& (newIndividualCheckedSet[7][thirdcol] == (ascv[5] & newIndividualCheckedSet[7][4])))
	{
		memcpy(tblock, block, 64);	//copy message block
		//block difference m2=+2^8, m14 = 2^31, cover 1 case
		tblock[2] += 0x00000100;
...
bool SetCheckAlgorithm(const uint32 ihv[], const uint32 cv[], const uint32 scv[],const uint32 ascv[], const uint32 block[]){
	if(DistinguishableSetCheckAlgorithm(ihv, cv, scv,ascv, block)){	//check Distinguishable Set
		return true;
	}
	if(IndividualSetCheckAlgorithm(ihv, cv, scv,ascv, block)){		//check Individual Checked Set
		return true;
	}
...
void CorrectCheckAlgorithm(){
	uint32 ihv[4],cv[4], scv[5] ={0},ascv[6] ={0};
	...
	Compression_Modify(ihv, cv, scv,ascv, b12_ref4);	//compress a block, output chaining values cv and sufficient condition vector
	if(SetCheckAlgorithm(ihv, cv, scv,ascv, b12_ref4)){
	...
	Compression_Modify(ihv, cv, scv,ascv, b22_ref4);	//compress a block, output chaining values cv and sufficient condition vector
	if(SetCheckAlgorithm(ihv, cv, scv,ascv, b22_ref4)){
		cout<<"ref  4 is OK, difference is m11 = +2^15, m4 = m14 = 2^31."<<endl;
	}
	...
	Compression_Modify(ihv, cv, scv,ascv, b13_ref21);	//compress a block, output chaining values cv and sufficient condition vector
	if(SetCheckAlgorithm(ihv, cv, scv,ascv, b13_ref21)){
	...
	Compression_Modify(ihv, cv, scv,ascv, b12_ref6);	//compress a block, output chaining values cv and sufficient condition vector
	if(SetCheckAlgorithm(ihv, cv, scv,ascv, b12_ref6)){
	...
	Compression_Modify(ihv, cv, scv,ascv, b22_ref6);	//compress a block, output chaining values cv and sufficient condition vector
	if(SetCheckAlgorithm(ihv, cv, scv,ascv, b22_ref6)){
		cout<<"ref  6 is OK, difference is m2 = +2^8, m11 = +2^15, m4 = m14 = 2^31."<<endl;
	}
...
//	ihv[0] = 0xb99e49d4, ihv[1] = 0x076c9442, ihv[2] = 0x77dfe793, ihv[3] = 0x03d49001;	//ihv 2
	...
	Compression_Modify(ihv, cv, scv,ascv, b11_ref16);	//compress a block, output chaining values cv and sufficient condition vector
	if(SetCheckAlgorithm(ihv, cv, scv,ascv, b11_ref16)){
	...
	Compression_Modify(ihv, cv, scv,ascv, b21_ref17);	//compress a block, output chaining values cv and sufficient condition vector
	if(SetCheckAlgorithm(ihv, cv, scv,ascv, b21_ref17)){
	...
	Compression_Modify(ihv, cv, scv,ascv, b11_ref17);	//compress a block, output chaining values cv and sufficient condition vector
	if(SetCheckAlgorithm(ihv, cv, scv,ascv, b11_ref17)){
		cout<<"ref 17 is OK, difference is m5 = +2^10, m10 = 2^31."<<endl;
	}
	...
	Compression_Modify(ihv, cv, scv,ascv, b22_ref18);	//compress a block, output chaining values cv and sufficient condition vector
	if(SetCheckAlgorithm(ihv, cv, scv,ascv, b22_ref18)){
	...
	Compression_Modify(ihv, cv, scv,ascv, b12_ref18);	//compress a block, output chaining values cv and sufficient condition vector
	if(SetCheckAlgorithm(ihv, cv, scv,ascv, b12_ref18)){
		cout<<"ref 18 is OK, difference is m6 = +2^8, m9 = m15 = 2^31."<<endl;
	}
	...
	Compression_Modify(ihv, cv, scv,ascv, b12_ref19);	//compress a block, output chaining values cv and sufficient condition vector
	if(SetCheckAlgorithm(ihv, cv, scv,ascv, b12_ref19)){
	...
	Compression_Modify(ihv, cv, scv,ascv, b22_ref20);	//compress a block, output chaining values cv and sufficient condition vector
	if(SetCheckAlgorithm(ihv, cv, scv,ascv, b22_ref20)){
	...
	Compression_Modify(ihv, cv, scv,ascv, b12_ref20);	//compress a block, output chaining values cv and sufficient condition vector
	if(SetCheckAlgorithm(ihv, cv, scv,ascv, b12_ref20)){
		cout<<"ref 20 is OK, difference is m9 = +2^27, m2 = m12 = 2^31."<<endl;
	}
	cout<<endl;
}
